...
python3 -m verl.trainer.main_ppo \
    ...
    trainer.experiment_name="qwen3-8b-taco-multi-turn-grpo-$(date +%Y%m%d-%H%M%S)" \
    trainer.nnodes=1 \
    trainer.n_gpus_per_node=4 \
    trainer.default_local_dir=~/qwen3-8b-taco-multi-turn-grpo \
    trainer.save_freq=10 \
    trainer.test_freq=10 \
    trainer.total_epochs=2 \
    reward_model.reward_manager=prime $@ 2>&1 | tee grpo_qwen.log
    
# bash train_grpo_multi_turn.sh