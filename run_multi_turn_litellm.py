...
async def infer_w_engine(conversations, engine, args):
    infer_tasks = []
...
async def process_one_example(example, engine, t=8): # train t=2; eval: t=8
    ...
    for _ in range(t):
        completions, usage_list = await infer_w_engine(conversations, engine, args)
...
async def process_one_example_wrapper(example, engine, f_handle, pbar):
    async with TASK_SEM:
        result = await process_one_example(example, engine)
...
async def process_all_examples():
    ...
    with open(args.output_file, "a", encoding="utf-8") as f, tqdm(total=len(data)) as pbar:
        for example in data:
            tasks.append(asyncio.create_task(process_one_example_wrapper(example, engine, f, pbar)))
